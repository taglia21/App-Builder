...
class TestPostBuild:
    def test_missing_idea_returns_422(self, client: TestClient) -> None:
        resp = client.post("/api/build", json={"theme": "Modern"})
        assert resp.status_code == 422

    def test_short_idea_returns_422(self, client: TestClient) -> None:
        resp = client.post("/api/build", json={"idea": "ab"})
        assert resp.status_code == 422

    def test_valid_build_returns_201_fields(self, client: TestClient) -> None:
        resp = client.post("/api/build", json={"idea": "A marketplace for vintage sneakers"})
...
class TestSSEContentType:
    def test_stream_sets_event_stream(self, client: TestClient) -> None:
        # Create a build first
        create_resp = client.post("/api/build", json={"idea": "Test idea for SSE streaming"})
