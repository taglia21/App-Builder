...
'''
    ...
    def _generate_configs(self, app_name: str, description: str):
        ...
        # SECURITY: Always generate a fresh key — never copy the host SECRET_KEY
        # or any other real credential from the running environment.
        import secrets
        secret_key = secrets.token_urlsafe(32)

        # .env with actual secure values (for immediate use)
        # SECURITY: Only placeholder or freshly-generated values are written here.
        # Real API keys (OPENAI_API_KEY, STRIPE_SECRET_KEY, etc.) are left blank
        # so the developer fills them in manually — they are NEVER read from
        # os.environ and copied into the generated output.
...
'''
    ...
    def _generate_docs(self, app_name: str, description: str, entity: Dict):
        ...
        # NOTE: .env.example is written by _generate_configs(); do not write it
        # again here to avoid overwriting the more complete version.
...
'''
    ...
    def _calculate_metrics(self):
        """Calculate generation metrics by counting actual lines in generated files."""
        import os
        self.metrics['total_files'] = len(self.files_created)

        # Count lines from in-memory tracking (fast, always available)
        content_lines = sum(f['lines'] for f in self.files_created)

        # Re-count from disk to get accurate line counts (handles edge cases
        # where content was post-processed or files were written externally).
        disk_lines = 0
        _counted_extensions = ('.py', '.html', '.css', '.js', '.ts', '.tsx',
                               '.yml', '.yaml', '.json', '.md', '.txt', '.sql')
        if self.output_dir.exists():
            for root, dirs, files in os.walk(str(self.output_dir)):
                for f in files:
        ...
        # Use disk count if available (more accurate), fall back to content count
        self.metrics['total_lines'] = disk_lines if disk_lines > 0 else content_lines
