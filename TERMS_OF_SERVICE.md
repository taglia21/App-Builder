# Terms of Service

**LaunchForge - AI-Powered Startup Builder**

*Last Updated: January 25, 2026*

## 1. Acceptance of Terms

By accessing or using LaunchForge ("Service"), you agree to be bound by these Terms of Service ("Terms"). If you do not agree, do not use the Service.

## 2. Description of Service

LaunchForge is an AI-powered platform that:
- Generates validated startup ideas using market intelligence
- Creates production-ready application code
- Provides business planning and validation tools
- Offers deployment automation

## 3. Account Registration

### 3.1 Eligibility
You must be at least 16 years old and capable of forming a binding contract to use LaunchForge.

### 3.2 Account Responsibility
- Provide accurate, complete registration information
- Maintain the security of your account credentials
- Notify us immediately of unauthorized access
- You are responsible for all activities under your account

## 4. Subscription and Payments

### 4.1 Pricing
- **Free Tier**: Limited generations per month
- **Pro Plan**: $29/month - Unlimited generations, priority support
- **Enterprise**: Custom pricing - Dedicated support, SLA

### 4.2 Billing
- Subscriptions are billed in advance monthly or annually
- Payments processed securely via Stripe
- Prices may change with 30 days notice

### 4.3 Refunds
- 14-day money-back guarantee for first-time subscribers
- No refunds for partial billing periods

## 5. Acceptable Use

### 5.1 You May:
- Generate startup ideas and applications for legitimate business purposes
- Modify and deploy generated code
- Use generated content commercially (subject to Section 6)

### 5.2 You May Not:
- Use the Service for illegal activities
- Generate harmful, malicious, or fraudulent content
- Attempt to reverse-engineer our AI systems
- Circumvent usage limits or security measures
- Resell or redistribute the Service
- Generate content that infringes third-party rights

## 6. Intellectual Property

### 6.1 Your Content
You retain ownership of:
- Input data you provide
- Business ideas and concepts you create
- Customizations to generated code

### 6.2 Generated Content
- Generated code is provided under MIT License
- You own derivative works you create
- We retain rights to underlying AI models and systems

### 6.3 Our Intellectual Property
LaunchForge, our logos, and Service design are our property.

## 7. AI-Generated Content Disclaimer

### 7.1 No Guarantee
AI-generated content is provided "as-is." We do not guarantee:
- Code correctness or bug-free operation
- Business viability of generated ideas
- Compliance with specific regulations
- Security of generated applications

### 7.2 Your Responsibility
You are responsible for:
- Reviewing and testing generated code
- Ensuring regulatory compliance
- Securing deployed applications
- Validating business assumptions

## 8. Limitation of Liability

THE SERVICE IS PROVIDED "AS IS" WITHOUT WARRANTIES OF ANY KIND, EXPRESS OR IMPLIED.

TO THE MAXIMUM EXTENT PERMITTED BY LAW:
- Our total liability is limited to amounts paid in the 12 months preceding the claim
- We are not liable for indirect, incidental, special, or consequential damages

## 9. Indemnification

You agree to indemnify and hold harmless LaunchForge from claims arising from:
- Your use of the Service
- Your violation of these Terms
- Applications you deploy using generated code

## 10. Term and Termination

### 10.1 Termination by You
- Cancel your subscription anytime
- Delete your account via settings
- Contact support@launchforge.io

### 10.2 Termination by Us
We may terminate or suspend your access for violation of these Terms.

### 10.3 Effect of Termination
Upon termination, you may export your data within 30 days.

## 11. Dispute Resolution

### 11.1 Governing Law
These Terms are governed by the laws of Delaware, USA.

### 11.2 Arbitration
Disputes shall be resolved through binding arbitration under AAA rules.

## 12. General Provisions

- These Terms constitute the entire agreement
- Our failure to enforce any right does not waive that right
- If any provision is unenforceable, the remainder stays in effect

## 13. Contact

- **Email**: legal@launchforge.io
- **Support**: support@launchforge.io

---

*By using LaunchForge, you acknowledge that you have read, understood, and agree to these Terms of Service.*
